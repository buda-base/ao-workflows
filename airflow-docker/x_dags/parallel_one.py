...
@task
def get_next_file(directory, **context):
    files = os.listdir(directory)
    bzs = [f for f in files if f.endswith('.bag.zip')]
    pp(f" next to run is {bzs[0]}")
    return bzs[0]
...
def create_subdag_tasks(parent_dag_id: str, **context):
    files = context['ti'].xcom_pull(key='file_list', task_ids='get_next_file')
    pp(files)
